(function(){
  ...
  function hide(){
    ...
    setTimeout(function(){if(loader)loader.classList.add('out');},400);
  }
...
if(cur&&curR){
  ...
  document.querySelectorAll('a,button,.card,.bt,.faq-q,.pill,.founder,.tl-item').forEach(function(el){
...
// ── CANVAS PARTICLES (or doré, discret) ─────────────────────────────────────
  ...
  function conn(){
    for(var i=0;i<pts.length;i++)for(var j=i+1;j<pts.length;j++){
      ...
      if(d<100){
        ...
        ctx.strokeStyle='rgba(201,162,39,'+(0.06*(1-d/100))+')';
        ctx.lineWidth=.3;
        ctx.stroke();
      }
    }
    ...
    fetch(form.action,{method:'POST',body:data,headers:{'Accept':'application/json'}})
      .then(function(r){
        if(r.ok){
          ...
          fmsg.textContent='Erreur. Écrivez directement à contact@chronosensiumtechnologies.com';
      ...
      .catch(function(){
        fmsg.textContent='Erreur réseau. Écrivez à mcsri@pm.me';
        fmsg.className='err';fmsg.style.display='block';
        if(submitBtn){submitBtn.disabled=false;submitBtn.textContent='Envoyer la demande';}
      });
  });
...
// ── KEYBOARD ESC — ferme menu mobile ─────────────────────────────────────────
